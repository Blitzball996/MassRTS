...
### 已完成（编译通过 + 冒烟测试）
- [x] **#滚轮**：滚轮恢复为**始终控制相机缩放**（含 sculpt 模式）；笔刷力度改用 `,` / `.` 键调整。
- [x] **#strength**：默认力度调低（0.35 → **0.15**），`,` 减 / `.` 加，范围 0.05~1.0。
- [x] **#笔刷光标椭圆**：圆环改成**相机朝向的 billboard**（用 view 矩阵 right/up 构建），在垂直洞壁上不再被拉成椭圆，始终保持圆形。
- [x] **#挖洞(4号笔刷)**：改成**盾构机(TBM)式连续掘进**——按住左键沿光标方向持续往深处推进，力度控制掘进速度（低力度=慢挖）。
- [x] **#bug.png 条纹/支离破碎/蓝色混淆**：根因是 `v_biome` 作为 float **在三角形间被插值**，跨越 depth 阈值的三角形让 biome 经过幻影 ID 2~6（水=蓝/沙/雪）→ 全图彩色条纹。修法：顶点**只存地表 biome ID（不再编码 7/8）**，泥土/岩石改由片元着色器从**连续的 depth 通道**(`v_height_norm`)选择，插值安全无条纹。
- [x] **#深处颜色**：strata 以**灰色为基底**渐变加深成深灰/炭灰（grass→dirt→rock），不再偏蓝。
- [x] **#上坡变慢**：`get_speed_mult` 坡度惩罚加强（系数 0.6→1.5，下限 0.6→0.35），爬陡坡降到约 35% 速度、冲下坡 +30%，高地优势成为真正的战术要素。

### 待办
- [ ] **#相机跟随入洞**：相机已是全手动 R/F 升降无下限，可进坑；如需"自动跟到所视平面最深处"再加。
- [ ] **#联网测试(listen-server)**：lockstep 框架(session.h)已就绪，已加 `CmdKind::Carve`；还需在 main_gpu 实例化 session、把挖掘/出兵/移动指令走 `queue_local_command`、并在 sim tick 用 `commands_for` 应用 → 实现一机双开(host+client)本地联机测试。GPU 17 万单位的确定性(RNG/浮点)需先验证以防 desync。
- [ ] **#沼泽**：Plan B 后低平区由统一 SDF 生成，条纹根因已修，待实跑确认沼泽是否还碎。
- [ ] **#12 无限延展地图**：World Partition / 学 Minecraft 分块流式（单独阶段）。

### 操作速查（按 B 进入挖掘模式）
- 数字键 1/2/3/4 = 升高土壤 / 挖坑 / 平滑 / 挖洞(TBM)
- `[` `]` = 笔刷半径 -/+
- `,` `.` = 笔刷力度 -/+
- 滚轮 = 相机缩放（任何时候）
- R/F = 相机升 / 降（可进坑仰视）
...
## SDFCraft 体素生存模式：性能 / 无缝行星 / 树木 / 网络（本轮会话）

> 目标对照 `docs/SDF_MINECRAFT_PORT_PLAN.md`：把"挖掘建造 + 无缝大行星 + 多人"逐步落地。
> 全部改动 **编译通过**，并新增 `test_sdfcraft` 头less 自测（树木/挖掘网格/网络收敛三项断言全过）。

### 已完成（编译 + 自测验证）
- [x] **#最强网格优化（卡顿根因）**：`mc_mesher.h` 重写。旧版 Marching Cubes 对**每个顶点**都重算
  连续 SDF 场两次（一次定等值面、一次做 6×8 采样法线）——每顶点上百次 5-octave 噪声，跨区域时
  一帧拉入整圈新 chunk 直接卡死。新版**先把 SDF 场一次性采样进本地稠密缓冲**（chunk + 1 体素梯度环），
  之后行进与法线（中心差分）全部走内存 O(1) 读取，零噪声。单 chunk 建网从数十毫秒降到亚毫秒。
  网格便宜后把每帧网格预算提到 6ms、生成预算提到 6 chunk/帧，跨区域解析更快且无尖峰。
- [x] **#挖着出现奇怪多边形**：典型 MC 伪影。解析地形场返回的是**以"块"为单位的距离**（可达 ±数十），
  而被雕刻的体素存的是 smin/smax 混合后**接近 0** 的值；两种量级混在一起做线性边插值会把顶点
  放到远离真实交点处 → 尖刺多边形，越挖越多。修法：采样时把场**钳制到窄带 [-1.5, 1.5]**，
  让所有样本同量级、插值良态。自测 `DIG MESH`：碰雕刻区域后 max_edge=1.48、long_edges=0（无尖刺）。
- [x] **#上坡摄像机抖动**：台阶式 step-up 每帧把 `pos.y` 离散抬升约 1 格，相机肉眼可见跳动。
  新增 `Player::render_eye()` + `update_camera_smoothing()`：物理/碰撞仍用精确 `pos`，**渲染用的
  竖直高度走临界阻尼跟随**（rate=12，落差>2格或飞行时直接吸附）。坡道/楼梯顺滑，无延迟感。
- [x] **#无缝宇宙到地面（行星背景）**：以前 LOD 球只在 G 总览模式存在，平时飞高只有一圈平地、
  四周是雾墙。现在普通视角下**先把 LOD 行星球当远景绘制**（行星中心置于本地海平面正下方 R 米，
  本地地块正好落在球面上），再清深度把本地地形画在最上层。低空只见微弱地平线弧度，爬升平滑揭示
  真实曲率与贴着太空的边缘弧（limb）。截图验证：3000m 处可见行星球体 + 顶部深空带，地面正常。
- [x] **#雾太大看不到渐变**：雾与相机远平面**随高度缩放**。地面照旧，爬升时雾 start/end 推到数十 km
  并逐渐变薄；天空与雾色从蓝渐变到深空蓝/黑，形成 地面→天空→轨道 连续渐变。雾距离改绑**实际
  加载视距**（view_radius×CHUNK_SX），不再写死 8 chunk。
- [x] **#树木升级（之前确实没生效，本轮重写并自测）**：`world.h` 把单一橡树换成**两种树种**。
  橡树：渐细树干(5~12)+ 椭球分层树冠 + 随机倾斜；松树（高/冷地或随机）：直树干穿过逐层收缩的
  叶环到尖顶。加入**林线**（雪峰以上不长树）。全部由世界坐标确定性生成。
  自测 `TREE GEN`：36 chunk 内 65 棵树（11 高松 + 54 橡）、8169 叶块、500 木块。
  （注：树木走方块 cube mesher 那条渲染路径写入 block data；若运行中看不到变化通常是旧窗口/exe 未重启，
   树密度约 0.7% 列，本就稀疏。）
- [x] **#飞行/行走按键分离**：`F`=切换飞行；新增 `R`=起飞(进飞行)、`C`=降落(进行走) 两个专用键；
  `G`=行星总览；`Ctrl`=飞行加速。`FrameInput` 加 `request_fly`/`request_walk`。
- [x] **#网络（按文档推进）**：新增 `src/sdfcraft/net_ops.h`——把 WorldOps 接缝做成真正可复制的
  编辑传输（server 权威 + client 预测模型，但用 World 表达，纯离线可单测）。
  - `EditMsg`（carve/place 的 POD 线格式）+ `encode/decode`（小端，与 net/core 一致字节序）
  - `EditTransport` 抽象双工通道 + `LoopbackTransport` 进程内双端（确定性、零 socket、可单测）
  - `EditReplicator`：本地立即应用 + 带 (author,seq) 序列广播；入站按 (author,seq) **去重**，
    回声广播不会重复应用。
  - `NetWorldOps::attachReplicator()` 接上后 carve/place 自动走复制、`tick()` 每帧 pump 入站远端编辑；
    不接则等同单机（纯本地预测），同一后端离线也能跑。
  - 自测 `NET`：两个同种子 World 经 loopback 交换 host 的 carve + client 的 place 后**收敛一致**，
    二次 pump 应用 0 条（幂等去重）。

### 测试设施（本轮新增）
- `src/main_sdfcraft.cpp` 加**离屏截图测试台**：`--shot <png> [--frames N] [--fly] [--planet]
  [--dig] [--pos x y z] [--look yaw pitch] [seed]`，隐藏窗口渲染 N 帧后用 stb_image_write 落盘，
  用于回归验证树木/雾渐变/挖掘伪影/行星背景。
- `src/sdfcraft/selftest.cpp`（`test_sdfcraft` 目标）新增三组断言：树木生成、挖掘网格无尖刺、网络收敛+去重。

### 待办（网络下一阶段）
- [x] **真实 UDP/TCP 后端**：`src/sdfcraft/net_transport_tcp.h` 新增 `TcpEditTransport`
      （复用 `src/net/core/tcp_socket.h`，4 字节小端长度前缀分帧 + 非阻塞 poll + 半包重组缓冲），
      `EditTransport` 现可跨进程/跨机收发 carve/place。`TcpNetInit` 做 RAII 的 WSAStartup。
      新增 `test_net_transport`（host+client 走 loopback 真 socket，双向复制 + 收敛 + 幂等断言全过）。
- [x] **chunk 流式同步**：新玩家进入时同步已存在的编辑增量（之前只复制实时编辑，不补历史）。
      `net_ops.h` 给 `EditReplicator` 加了**有序编辑回放日志** `history_`：本机 `carve()/place()`
      与 `pump()` 收到并应用的远端编辑都按 `(author,seq)` 顺序追加。新增 `snapshot()`（把整段历史
      序列化成一个 blob）+ `applySnapshot()`（晚加入者解码并按 `(author,seq)` 去重后逐条重放，
      与同步期间撞车的实时编辑不会重复应用，重复喂同一快照为 no-op）。世界由种子确定性生成，只有
      玩家编辑会发散，故回放历史即可重建已雕刻/已建造的世界。自测 `NET BACKFILL`：晚加入者重放
      全部历史编辑后，方块与 SDF 场逐点等于 host，且二次 applySnapshot 返回 0。
- [ ] **冲突/回滚**：client 预测被 server 拒绝时的 rollback+replay（接缝已留 `isAuthoritative()`）。
- [ ] **玩家状态复制**：位置/朝向/挖掘动画（目前只复制世界编辑，不复制玩家实体）。

### 已修复（本轮：shader 还原）
- [x] **`sdfcraft_chunk.frag` 被改坏 → 还原平滑版**：工作区里该 frag 被替换成 "Genshin 风格化"
      版本，问题有二：(1) 材质码与 `blocks.h` 不同步（水被当成木头渲染等）；(2) 用硬
      `if (mat==X)` 分块上色，导致材质交界处出现折角/接缝/奇怪多边形。已 `git checkout` 还原为
      已提交的三平面采样 + 连续 slope/depth 混合 + 分数材质混合版本（无接缝、无折角），并同步到
      `build/Release/shaders/`。`test_sdfcraft` 的 `DIG MESH: long_edges=0` 确认挖掘网格无尖刺多边形；
      headless 截图（地表 + 挖掘剖面）目视过渡平滑。

### 已修复（本轮：树木交互 + 树形 + 挖掘多边形复核）
- [x] **树木不能交互（根因）**：`player.h` 的 `raycast()` 只步进 `sample_sdf`（平滑地形场），
      而树（LOG/LEAVES）是不在 SDF 场里的离散方块，射线直接**穿过**树打到后面的地面，所以
      永远选不中树。改为**双轨步进**：在沿射线步进 SDF 的同时并行步进体素栅格，一旦踏入"实心且
      非地形"方块（树/木板/玩家放置块）立即命中并由步进轴反推命中面。这样树可被选中/砍伐/作为
      放置参考面——`do_dig()/do_place()` 本就支持离散方块，只是之前射线从不返回树命中。
      自测 `TREE RAYCAST: hit=1 block=6(LOG)` 确认从侧面看树会命中树干而非穿透到地面。
- [x] **树丑 + 空心**：旧 `build_oak/build_pine` 用**几层薄圆盘**堆叠成树冠（半径 2,3,3,2,1），
      层与层之间留斜向台阶缝隙→侧看能透过缝看到天空/树干，读作"空心 + 难看"。重写为**实心 3D 体**：
      oak 用带逐体素噪声抖动轮廓的**椭球树冠**（随树干 lean 漂移），pine 用**密堆叠锥体**（每层实心圆盘
      平滑收窄、相邻层重叠无透视缝、双块尖顶）。树冠更宽，故 `maybe_tree` 边界留空从 4 提到 6 防裁切。
      自测 `TREE GEN: logs=205 leaves=3745`（叶量大增、叶>干）。
- [x] **挖掘奇怪多边形复核**：`DIG MESH: long_edges=0` 仍为 0（重叠球雕刻后无尖刺/杂多边形），
      确认上一轮 shader 还原 + MC 网格化已根除该现象；本轮无回归。

### 操作速查（SDFCraft 生存模式）
- `WASD`+鼠标 = 移动/视角；`Space`/`Shift` = 跳/蹲（飞行时升/降）
- `F` = 切换飞行；`R` = 起飞；`C` = 降落；`Ctrl` = 飞行加速
- `G` = 行星总览视图
- 左键 = 挖；右键 = 放置；`1`~`9` = 物品栏
