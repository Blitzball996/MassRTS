#pragma once
// =============================================================================
// SDFCraft - Crafting screen (R key, Minecraft Console Edition recipe list)
// -----------------------------------------------------------------------------
// Console Edition specific feature: shows a scrollable list of available recipes.
// Click a recipe row to craft it automatically (if you have the ingredients).
// This is different from PC Edition's manual grid placement.
//
// Based on MinecraftConsoles UIScene_CraftingMenu.cpp
// =============================================================================
#include <glad/glad.h>
#include <glm/glm.hpp>
#include <vector>
...
public:
    ...
    void draw(Inventory& inv, RecipeBook& recipes, int fbw, int fbh, float mx, float my) {
        ...
        // Panel
        quad(panel_.x, panel_.y, panel_.z, panel_.w, glm::vec4(0.1f,0.1f,0.12f,0.98f));

        // Title bar
        quad(panel_.x, panel_.y, panel_.z, 16*s_, glm::vec4(0.18f,0.18f,0.22f,1));
        draw_text("CRAFTING", panel_.x + 8*s_, panel_.y + 4*s_, 1.2f, glm::vec4(1,1,1,1));
        ...
        for (int vis = 0; vis < rows_visible_; vis++) {
            ...
            bool hover = (mx>=r.x && mx<r.x+r.z && my>=r.y && my<r.y+r.w);

            glm::vec4 bg = ok ? (hover ? glm::vec4(0.28f,0.32f,0.28f,1) : glm::vec4(0.16f,0.18f,0.16f,1))
                              : glm::vec4(0.14f,0.12f,0.12f,1);
            quad(r.x, r.y, r.z, r.w, bg);

            // Result icon on left
            float ic = 16*s_, pad = 3*s_;
            icon(r.x+pad, r.y+pad, ic, ic, rc.result.id);
            draw_count(r.x+pad, r.y+pad, ic, ic, rc.result.count);

            // Arrow
            float ax = r.x + pad + ic + 4*s_;
            draw_text("->", ax, r.y+pad+2*s_, 0.8f, glm::vec4(0.7f,0.7f,0.7f,1));

            // Ingredients on right
            float ix = ax + 16*s_;
            for (auto& nd : RecipeBook::recipe_needs(rc)) {
                icon(ix, r.y+pad, ic, ic, nd.id);
                draw_count(ix, r.y+pad, ic, ic, nd.count);
                ix += ic + 2*s_;
            }
        }
...
private:
    ...
    int scroll_=0, rows_visible_=0;
    float row_h_=0;

    void layout(int fbw, int fbh) {
        s_ = std::max(2.0f, fbh/240.0f);
        float pw = 240*s_, ph = 200*s_;
        panel_ = {(fbw-pw)*0.5f, (fbh-ph)*0.5f, pw, ph};
        row_h_ = 24*s_;
        rows_visible_ = (int)((ph - 20*s_) / row_h_);
    }

    glm::vec4 row_rect_(int vis) const {
        return {panel_.x+4*s_, panel_.y+20*s_ + vis*row_h_, panel_.z-8*s_, row_h_-2*s_};
    }
