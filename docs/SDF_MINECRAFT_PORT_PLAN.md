...
- [ ] 阶段 O：UI / HUD / 音效 / 打磨
- [ ] 阶段 P：行星尺度（八叉树 LOD → 地球模拟 / 太空-地面无缝）⭐ 用户要求
...
### 1.4 空间分割与 LOD：八叉树地形 + 行星尺度无缝（⭐ 用户要求，面向地球模拟）

> 目标：当前是平铺的 `unordered_map<(cx,cz), Chunk>` 固定分辨率区块，**玩家走一会卡顿**的根因之一就是
> 它每帧把视野圆盘内所有缺失区块同步生成。长期还要支持**地球模拟 + 太空高空无缝连接到地面**，
> 平铺固定 LOD 的方案在行星尺度下会爆内存、爆三角面、无法表达从轨道到地表的尺度跨度。
> 因此引入 **八叉树（Octree）空间分割 + 距离驱动 LOD**，并预留**行星坐标系**。

#### 1.4.1 为什么是八叉树（而不是 16/10 叉树）

| 方案 | 子节点数 | 评价 |
|------|---------|------|
| 四叉树 Quadtree | 4（仅 XZ 分） | 只适合 2.5D 高度场，**无法表达洞穴/悬垂/行星曲率**，否决 |
| **八叉树 Octree** | **8（XYZ 三轴二分）** | **3D 空间均匀二分，天然适配 SDF 三维体素 + Marching Cubes，业界标准（含行星引擎）** ✅ |
| 16/10 叉树（N 叉） | 16 / 10 | 非 2 的幂或非立方拆分，无法整齐三轴二分；分支因子大反而让单层节点更新成本高、缓存局部性差，**收益为负**，否决 |

**结论：八叉树是正解。** 16/10 叉树是误区——空间分割的关键不是分支多，而是"每层把空间沿三轴各二分一次"，
这正好是 8。更大的分支因子只会让树更扁但每个节点更新更贵，且无法对齐 2^n 体素边界（破坏 Morton 编码与 GPU 对齐）。

> 行星地形领域的成熟做法（如 No Man's Sky、Outerra、Star Citizen 的行星）几乎都是
> **八叉树 / 自适应 SDF 八叉树** + 距离 LOD，验证了选型。

#### 1.4.2 优化作用分析（能否解决"走一会卡住"+ 支撑地球模拟）

**A. 解决卡顿（近期收益，直接相关）**
- 当前：固定分辨率平铺区块，视野内所有区块同分辨率，跨边界一次性涌入大量同精度区块 → 生成/网格化峰值卡顿。
- 八叉树后：**近处高精度（小叶子节点）、远处低精度（大节点）**，远处一个大节点代替成百上千个细区块，
  - 生成/网格化的**节点总数随距离对数增长**而非线性 → 同视距下需要处理的节点数下降 1~2 个数量级
  - 配合已加的"每帧生成预算"，峰值被彻底摊平
- 视锥剔除：八叉树天然支持层级剔除，背后/视锥外整棵子树一次跳过。

**B. 支撑地球模拟 + 太空到地面无缝（长期收益，核心理由）**
- 行星半径 ~6371km，体素若 1m，直接平铺需 ~10^21 体素，**任何平铺方案都不可能**。
- 八叉树按需细分：从轨道看整个行星只需顶部几层节点（粗 LOD 球面），俯冲到地表时**沿相机路径动态细分**到米级，
  - 内存/三角面**只跟"当前可见细节"成正比**，与行星绝对尺寸无关 → 这是无缝太空-地面的唯一可行解
- 配合**浮点原点重定位（floating origin）**：相机始终在原点附近，避免远离原点时 float 精度爆炸（行星尺度必须）。
- LOD 之间用 SDF 的天然优势做**无接缝过渡**（SDF 在不同分辨率间插值连续，配合 Transvoxel/skirt 消除裂缝）。

**C. 风险与代价（诚实评估）**
- 复杂度显著上升：邻居查找、跨 LOD 边界的网格裂缝（crack）需要 **Transvoxel 算法**或 skirt 补丁处理。
- 不是免费的：实现成本高，需要分阶段。**近期只需引入八叉树 + 距离 LOD 解决卡顿**；
  行星坐标系/floating origin 留到"地球模拟"专门阶段（见下方阶段 P），不阻塞当前 MVP。

#### 1.4.3 落地设计

| 要素 | 设计 |
|------|------|
| 节点 | `OctreeNode{ AABB bounds; int lod; SDFChunk* data; OctreeNode* children[8]; }` |
| 细分准则 | 节点到相机距离 < k × 节点尺寸 → 细分；反之合并（带滞回，防抖动） |
| 叶子分辨率 | 叶子 = 当前 32³ 体素的 `Chunk`（复用现有 world/mesher/mc_mesher） |
| 编码 | Morton（Z-order）编码节点坐标，缓存友好 + 便于 GPU 上传 |
| 裂缝处理 | 相邻不同 LOD 节点边界用 **Transvoxel**（首选）或 skirt（先用 skirt 顶住） |
| 生成 | 仍 per-node 惰性生成，受"每帧预算"约束；远 LOD 用更高频率噪声八度截断 |
| 行星坐标（阶段 P） | 立方球（cube-sphere）6 面各一棵八叉树；floating origin 重定位相机 |
| 网络 | 节点按 Morton key 同步；AOI 按 LOD 分级推送（近处全量、远处粗节点） |

**实现顺序（不阻塞当前玩法）：**
1. **近期（紧接当前修复）**：把平铺 `chunks_` 升级为八叉树管理 + 距离 LOD，叶子复用现有 Chunk。先用 skirt 防裂缝。目标：消除大视距卡顿，验证 LOD 过渡。
2. **中期**：Transvoxel 替换 skirt，做无缝 LOD 边界。
3. **远期（阶段 P）**：cube-sphere 行星坐标 + floating origin，实现太空-地面无缝。

> **与第 0 节进度总览的关系**：新增 **阶段 P：行星尺度（地球模拟 / 太空-地面无缝）**，
> 但其前置（八叉树 + LOD）作为"技术基础"在当前阶段 B 收尾时引入，因为它直接解决卡顿。
...
> 目标：从"无尽平面世界"演进到"行星世界"，支持从太空轨道俯冲到地表的**无缝尺度过渡**。
> 技术核心见 1.4 节。本阶段拆成三步，前两步也直接服务于"走一会卡顿"的根治。

### P1. 八叉树 + 距离 LOD（技术基础，提前到 B 阶段收尾引入）
- [ ] `OctreeNode` 结构 + 距离驱动细分/合并（带滞回防抖）
- [ ] 平铺 `chunks_` 迁移到八叉树管理，叶子复用现有 32³ `Chunk` / `mesher` / `mc_mesher`
- [ ] Morton（Z-order）节点编码，缓存友好
- [ ] 层级视锥剔除（整棵子树一次性剔除）
- [ ] LOD 边界裂缝：先用 **skirt（裙边）** 顶住
- [ ] 配合已实现的"每帧生成预算"，验证大视距无卡顿

### P2. 无缝 LOD 边界（Transvoxel）
- [ ] 用 **Transvoxel** 算法替换 skirt，做相邻不同 LOD 间无裂缝过渡
- [ ] 远 LOD 噪声八度截断（远处低频、近处补高频细节），保证细分时形状连续

### P3. 行星坐标系（地球模拟 / 太空-地面无缝）
- [ ] **Cube-sphere**（立方球）：6 个面各一棵八叉树，映射到球面，表达整个行星
- [ ] **Floating origin**（浮点原点重定位）：相机恒在原点附近，避免行星尺度 float 精度崩溃
- [ ] 大气/海平面/曲率渲染，轨道视角到地表视角的相机与 LOD 联动
- [ ] 行星级地形生成（大陆/海洋/山脉的低频基底 + 局部高频细节，复用 J 阶段生物群系作为表层）
- [ ] **网络：** 节点按 Morton key + LOD 分级 AOI 同步（近处全量、远处粗节点）

**P 阶段自测：** 从太空俯视行星（粗 LOD 球面）平滑俯冲到地表米级细节无接缝、无卡顿、无精度抖动；
多人各自位置独立 LOD 流，远近玩家都正确同步。
